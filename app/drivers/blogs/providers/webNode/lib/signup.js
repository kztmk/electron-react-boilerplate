...
const signup = async (blogInfo, opts) => {
  ...
  try {
    ...
    await page.waitFor('.wnd-welcome');
    ...
    new Noty({
        type: 'success',
        layout: 'topLeft',
        text:'[利用規約に同意しWebnodeIDへ登録する]をクリック' 
      }).show();
    `);
      await page.click('input[name="Submit2"]');
      log.info('[利用規約に同意しWebnodeIDへ登録する]をクリック');
    ...
    `);
      ...
      if (successMessage === 'WebnodeIDの登録完了') {
        log.info('WebnodeID登録完了');
        // サービスの追加リンクをクリック
        await page.addScriptTag({ path: notyJsPath });
        await page.addStyleTag({ path: notyCssPath });
        await page.addStyleTag({ path: notyThemePath });
    ...
    `);
        await page.click('a[href^="https://secure.id.Webnode.com/add.php"]');
    ...
    `);
      ...
      reverseButtons: true
        ...
        log.warn('WebnodeIDの登録完了ページが確認できません。');
        throw new Error('WebnodeIDの登録完了ページが確認できません。');
      }
  } catch (error) {
