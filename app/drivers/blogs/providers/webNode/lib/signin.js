...
const signin = async (blogInfo, opts) => {
  ...
  log.info('--------->login to  Webnode blog account--------->');
  log.info('-----------user----------');
  log.info(blogInfo);
  log.info('-------------------------');
  ...
  try {
    ...
    await page.goto(`https://Webnode.com/login.php?ref=blog`, { waitUntil: 'load' });

    log.info('access: https://Webnode.com/login.php?ref=blog');
